        ...
        try {
            ...
            case 'delete_user':
                ...
                try {
                    ...
                    $stmt->bind_param("i", $userId);
                    if (!$stmt->execute()) {
                        throw new Exception('Failed to delete user permissions');
                    }
                    ...
                    $stmt->bind_param("i", $userId);
                    if (!$stmt->execute()) {
                        throw new Exception('Failed to delete activity logs');
                    }
                    
                    // Delete file uploads
                    $stmt = $conn->prepare("DELETE FROM file_uploads WHERE user_id = ?");
                    $stmt->bind_param("i", $userId);
                    if (!$stmt->execute()) {
                        throw new Exception('Failed to delete file uploads');
                    }
